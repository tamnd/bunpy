---
title: "bunpy v0.10.x: package manager reaches uv parity"
date: 2026-04-28
description: Twenty-nine patch releases, six performance rungs, eight parity gaps closed, and one lockfile format dropped. What happened in v0.10.x.
---

v0.10.x started as a performance sprint and turned into a full parity push. By the time we shipped v0.10.29, the bunpy package manager resolved and installed packages as correctly as uv for the vast majority of real-world projects, and faster on a warm cache.
...
The performance story is covered in detail in the [pm lock benchmark post](/blog/v0-10-perf). The short version: we were 10x slower than uv at the start of v0.10.x. Six root causes, six fixes.
...
Performance is only useful if the output is correct. During the same period we found and fixed eight places where bunpy's resolver or installer produced different results than uv.
...
**G-2: Yanked releases included.** PyPI marks some releases as yanked (usually due to a critical bug). uv excludes yanked releases from resolution unless the user explicitly pins them. We were including them. Fixed by checking the `yanked` field in the PyPI JSON response.
...
**G-7: Extras in lockfile format wrong.** Our `uv.lock` output encoded extras incorrectly — we wrote them as a flat list rather than as part of the package identifier. `uv sync` would reject the lockfile. Fixed when we rewrote `WriteLockfile` to match the format exactly.
